...
fn init_schema(conn: &rusqlite::Connection) {
    ...
    let exists: i64 = conn
        .query_row("SELECT COUNT(*) FROM migrations WHERE id = ?1", params![baseline_id], |row| row.get(0))
        .unwrap_or(0);
    if exists == 0 {
        let ts = chrono::Utc::now().to_rfc3339();
        conn.execute("INSERT INTO migrations (id, applied_at) VALUES (?1, ?2)", params![baseline_id, ts]).ok();
    }
}
...
pub fn hash_key(key: &str) -> String {
    ...
    argon2.hash_password(key.as_bytes(), &salt).map(|ph| ph.to_string()).unwrap_or_else(|_| key.to_string())
}
...
fn seed_data(conn: &rusqlite::Connection) {
    ...
    let count: i64 = conn.query_row("SELECT COUNT(*) FROM workflows", [], |row| row.get(0)).unwrap_or(0);
    if count > 0 { return; }
    ...
    // ── 1. Workflows
    let workflows = vec![
        ("wf_1", "Automated Triage & Support Resolution", "Classifies incoming Zendesk tickets, analyzes sentiment, and drafts responses using specific product documentation.", "active", 42801),
    ...
    for i in 1..=85 {
        ...
        let w_idx = (i % 10) + 1;
        let severity = if i % 15 == 0 { "high" } else if i % 5 == 0 { "medium" } else { "low" };
        ...
        let meta = if severity == "high" { r#"{"error":"HTTP 429 Too Many Requests - Rate Limit Exceeded","action":"aborted"}"# } else { r#"{"status":"success","duration_ms":1450}"# };
    ...
    conn.execute("INSERT INTO api_keys (id, name, key_hash, scopes, status) VALUES (?1,?2,?3,?4,?5)", params!["key_1", "Production API Sync", hashed1, "read,write,execute", "active"]).ok();
    conn.execute("INSERT INTO api_keys (id, name, key_hash, scopes, status) VALUES (?1,?2,?3,?4,?5)", params!["key_2", "Staging Environment", hashed2, "read,write", "active"]).ok();
    conn.execute("INSERT INTO api_keys (id, name, key_hash, scopes, status) VALUES (?1,?2,?3,?4,?5)", params!["key_3", "Developer Key (Sarah)", hashed3, "read,write,execute", "active"]).ok();
    ...
    conn.execute("INSERT INTO integrations (id, provider, config, status) VALUES (?1,?2,?3,?4)", params!["intg_1", "OpenAI", r#"{"apiKey":"sk-..."}"#, "connected"]).ok();
    conn.execute("INSERT INTO integrations (id, provider, config, status) VALUES (?1,?2,?3,?4)", params!["intg_2", "Slack", r#"{"workspace":"acme-corp"}"#, "connected"]).ok();
    conn.execute("INSERT INTO integrations (id, provider, config, status) VALUES (?1,?2,?3,?4)", params!["intg_3", "GitHub", r#"{"org":"acme-corp"}"#, "connected"]).ok();
    conn.execute("INSERT INTO integrations (id, provider, config, status) VALUES (?1,?2,?3,?4)", params!["intg_4", "Zendesk", r#"{"subdomain":"acme.zendesk.com"}"#, "disconnected"]).ok();
...
mod tests {
    ...
    fn temp_db_path() -> String {
        ...
        p.push(format!("mermaduckle_test_{}.db", uuid::Uuid::new_v4().to_string()));
    ...
    fn test_migrations_and_seed() {
        ...
        if Path::new(&db_path).exists() { std::fs::remove_file(&db_path).ok(); }
        let pool = create_pool(&db_path);
        let conn = pool.get().unwrap();

        // migrations table should have at least the baseline entry
        let count: i64 = conn.query_row("SELECT COUNT(*) FROM migrations", [], |r| r.get(0)).unwrap_or(0);
        assert!(count >= 1, "migrations baseline missing");

        // integrations seeded
        let intg_count: i64 = conn.query_row("SELECT COUNT(*) FROM integrations", [], |r| r.get(0)).unwrap_or(0);
        assert!(intg_count >= 1, "integrations not seeded");

        // api_keys should contain hashed entries (argon2 format)
        let key_hash: String = conn.query_row("SELECT key_hash FROM api_keys LIMIT 1", [], |r| r.get(0)).unwrap();
        assert!(key_hash.contains("$argon2"), "api_keys not hashed with argon2");
    ...
    fn test_hash_key_verify() {
        ...
        assert!(Argon2::default().verify_password(raw.as_bytes(), &parsed).is_ok());
    }
}
