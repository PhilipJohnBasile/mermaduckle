...
pub async fn self_heal_node(pool: web::Data<DbPool>, body: web::Json<HealRequest>) -> HttpResponse {
    ...
    let (logs_json, workflow_id) = match conn.query_row(
        ...
        Ok(v) => v,
        Err(_) => return HttpResponse::NotFound().json(serde_json::json!({"error": "Run not found"})),
    };
    ...
    let target_node = nodes.iter().find(|n| n.get("id").and_then(|v| v.as_str()) == Some(node_id));

    if target_node.is_none() {
        return HttpResponse::NotFound().json(serde_json::json!({"error": "Node not found in workflow"}));
    }
    ...
    let context = format!("
        ...
        Current Config: {}
        Logs: {}
    ", target_node.unwrap().get("type").unwrap_or(&serde_json::json!("unknown")), target_node.unwrap().get("config").unwrap_or(&serde_json::json!("{}")), logs_json);
    ...
    match call_ollama(&ollama_url, "llama3.2", prompt).await {
        ...
        Err(e) => HttpResponse::InternalServerError().json(serde_json::json!({"error": format!("Self-healing connection failed: {}", e)}))
    }
}
